...
// 固定 fixtures（版本化、可复用）
const FX = JSON.parse(fs.readFileSync(path.join(ROOT, 'test/fixtures/presets.json'), 'utf8'));
...
// ── mock 上游：模型输出从 fixture 读取（模拟「每周工作4天」）──
const MODEL = JSON.stringify(JSON.parse(fs.readFileSync(path.join(ROOT, 'test/fixtures/model-wash.json'), 'utf8')));
...
try {
  ...
  const rr = await (await api(off, '/api/extract', { text: FX.presets.lemon })).json();
  ...
  const b1 = await (await api(ai, '/api/extract', { text: FX.cases.weekDaysNoWeeks })).json();
  ...
  const b2 = await (await api(ai, '/api/extract', { text: FX.cases.weekDaysWithWeeks })).json();
  ...
  const w1 = await (await api(ai, '/api/extract', { text: FX.cases.wageNoEvidence })).json();
  ok(!('wage' in (w1.fields || {})), '无「时薪」证据 → wage 被丢弃');
  const w2 = await (await api(ai, '/api/extract', { text: FX.cases.wageWithEvidence })).json();
  ...
  const d2 = await (await api(demo, '/api/extract', { text: FX.presets.lemon })).json();
  ...
  const kbR = await (await api(ai, '/api/extract', { text: FX.presets.wash })).json();
